...
def _restore_or_build_system_prompt(agent, system_message, conversation_history, query_text: str = None):
...
def run_conversation(
    ...
    # prefix cache.
    # Force rebuild on every turn if semantic search is enabled to allow per-message skill retrieval
    try:
        from hermes_cli.config import load_config as _load_config
        _cfg = _load_config()
        _ss = (_cfg.get("skills") or {}).get("semantic_search") or {}
    ...
    if agent._cached_system_prompt is None:
        _restore_or_build_system_prompt(agent, system_message, conversation_history, query_text=user_message)
